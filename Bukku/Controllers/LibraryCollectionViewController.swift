...
//  Bukku
//
//  Created by 김응철 on 2022/06/30.
//

import UIKit
import SnapKit

// MARK: - ViewModel -> LibraryViewModel
class LibraryCollectionViewController: UIViewController {
    ...
    // MARK: - LifeCycle
    init(_ libraryType: LibraryType?) {
...
extension LibraryCollectionViewController: UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let book = viewModel.getBookFromIndex(libraryType, index: indexPath.row)
        cell.configureImage(book)
        return cell
    }
    
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return viewModel.numberOfItemsInSection(libraryType)
    }
}
